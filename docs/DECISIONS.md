...
格式：在 _context_ 下，面对 _concern_，我们选择 _option_，接受 _tradeoff_，以获得 _benefit_。

1. 在单机 demo 语境下，面对"任务必须与客户端解耦"的要求，选择 **Redis list 做队列 + 独立 worker 进程**，
   接受多一个组件，以获得提交后可断连、可并发、可水平加 worker。
2. 在事件流场景下，面对单向 server→client 推送，选择 **SSE**，接受取消要另走 HTTP 接口，
   以获得原生断点续传（`Last-Event-ID`）与代理友好（见 `adr/0002`）。
3. 在租约实现上，面对"并发上限"与"故障重调度"两个需求，选择 **把租约本身当并发槽**
   （`SET lease:{id} NX EX ttl`），接受 TTL 需要与心跳周期协调，以获得两个需求共用一套机制。
4. 在终态写入上，面对"僵尸 worker 复活后覆盖新 attempt 结果"的风险，选择 **store 层对
   `(status=running, attempt=N)` 做 CAS**，接受写入可能无声失败，以获得旧 attempt 无法污染新结果。
5. 在工具面设计上，面对"够用"与"可审计"的矛盾，选择 **bash + read_file + write_file + list_dir 四件套**，
   接受 bash 使白名单不构成安全边界，以获得最小完备工具面（安全边界由容器承担，见 Q5/Q6）。
6. 在工具输出截断上，面对上下文预算，选择 **按工具类型分别截断**（read_file 取头部、bash 取尾部，约 50KB），
   接受可能丢失中段，以获得报错信息（通常在尾部）不被截掉。
7. 在 LLM 接入上，面对"评审者未必有 GPU 或 key"，选择 **mock 回放为默认、真实调用同码路可选**，
   接受 mock 不能证明自适应能力，以获得零依赖可复现的 demo（证据链见 Q7）。
8. 在真实端点不可用时，面对"演示必须跑通"的诱惑，选择 **直接失败并报错，禁止静默回落 mock**，
   接受 demo 可能跑不起来，以获得输出永远可信（见 Q8）。
9. 在 worker 崩溃恢复上，面对 agent 中间状态难以快照，选择 **attempt 级整体重跑**（`max_attempts=2`），
   接受重复消耗 token 与时间，以获得诚实且可解释的恢复语义（见 `adr/0003`）。
10. 在依赖管理上，面对系统 Python 为 3.10 且 `ensurepip` 缺失，选择 **uv 管理的 3.12 虚拟环境**，
    接受多一个工具依赖，以获得与容器内一致的解释器版本。
